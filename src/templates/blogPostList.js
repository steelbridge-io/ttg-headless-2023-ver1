...
import PageBanner from "@/components/news-banner";
...
const postConst = ({pageContext}) => (
	...
	<Layout PageTitle={pageContext.title} PageDescription={'The Trinity Guide Co. - News'}>
		<HeaderOne />
			...
			<Pagination className="container">
				{Array.from({length: pageContext.numberOfPages}).map((page, index) =>(
					<PageNumberWrapper key={index} isCurrentPage={index + 1 === pageContext.currentPage} >
						<PageNumber to={index === 0 ? '/fishing-news' : `/fishing-news/${index + 1}`}>
							{index + 1}
						</PageNumber>
